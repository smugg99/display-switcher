...
if [ $# -eq 0 ]; then
    ...
    if [ "$1" == "1" ]; then
        ...
        echo "Invalid argument. Please specify '1' for Display 1, '2' for Display 2, or 'all' to toggle both displays."
    fi
elif [ $# -eq 2 ] && [ "$1" == "all" ]; then
    toggle_input_source_display_1
    toggle_input_source_display_2
else
    echo "Invalid number of arguments or combination of arguments."
fi
